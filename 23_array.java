/*
В компании InvestTrade проходит ежегодное мероприятие, посвященное борьбе за звание «Трудолюб года» («The most hardworking employee of the year»). 
Диплом первой степени вручается тому из сотрудников, который за весь год провел на рабочем месте наибольшее количество часов. 
Вручаются дипломы только I, II и III степеней. Всего в конкурсе участвует count сотрудников.

Создай публичный класс Task, в котором будут реализованы следующие два статических метода:

1) Напиши публичный метод findAllWinners, который бы получал на вход массив типа double размера count, 
который будет содержать количество часов, которые count сотрудников провели на рабочем месте за этот год, а также возвращал бы время, 
которое каждый из призеров в номинации «Трудолюб года» провел на работе.
2) Напиши публичный метод peopleChoiceAward, который бы получал на вход два параметра: 
общий список участников конкурса и общее количество победителей конкурса. 
Данный метод должен находить порядковый номер первого попавшегося конкурсанта, который провел на рабочем месте ровно 1500 часов в этом году, среди тех людей, 
которые не являются призерами.

Предоставь исходный код класса Task.

Примечания:

При решении поставленной задачи для доступа к элементам массивов используй метод getDouble класса Array.
Для получения массива часов всех победителей конкурса используйте метод copyOfRange() класса Arrays.
