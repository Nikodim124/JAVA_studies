/*
В операционный офис компании PinApple из регионального представительства в Китае поступили сведения о клиентах компании. 
Операционному офису PinApple требуется выбрать всех клиентов, являющихся юридическими лицами, и передать их в формате CSV 
в аналитический отдел для подготовки коммерческих предложений.
 
Из-за того что данные поступили смешанными (в данных присутствуют как физические лица, так и юридические), 
специалисты операционного офиса не могут оперативно выполнить поставленную задачу, поэтому они обратились в отдел автоматизации с просьбой реализовать утилиту, 
которая бы позволила решать данную задачу максимально быстро.
...
5. В пакете com.intellekta.pinapple определи класс ClientClassifier. В классе определи публичный статический свободный метод getClientsByType, 
...
6. В методе getClientsByType реализуй алгоритм, который добавляет в массив result всех клиентов из массива clients, 
тип которых соответствует типу, переданному в метод (type).
...
Если в процессе выполнения алгоритма, количество искомых клиентов будет больше, чем размер массива result, 
выведи на экран значение «Result array length exceeded». Массив результатов должен содержать первых клиентов из массива clients, соответствующих заявленному типу.
