...
2.  В классе должен быть реализован конструктор без параметров, который будет собирать данные о пользователе с помощью консоли и заполнять поля объекта данными, 
...
если пользователь ввел в качестве Interest rate значение, не являющееся действительным положительным числом, 
выводить предупреждение: «Interest rate is incorrect. It must be strictly positive number»;
если пользователь ввел в качестве Credit amount значение, не являющееся действительным положительным числом, 
выводить предупреждение: «Credit amount is incorrect. It must be strictly positive number»;
если пользователь ввел в качестве Loan maturity значение, не являющееся целым положительным числом, 
выводить предупреждение: «Loan maturity is incorrect. It must be strictly positive int».
